...
echo "========================================================================="
echo ""
echo "  JBoss Modules Demo  Environment"
echo ""
...
if [ "x$LAUNCH_DEMO_IN_BACKGROUND" = "x" ]; then
    ...
    eval \"$JAVA\" $JAVA_OPTS \
        ...
        -jar \"$DEMO_HOME/jboss-modules-1.3.3.Final.jar\" \
        -mp \"$DEMO_HOME/modules\" \
        org.jboss.modules.quickstart \
        "$@"
    DEMO_STATUS=$?
    echo "JBoss Modules Demo Status: $DEMO_STATUS"
else
    ...
    eval \"$JAVA\" $JAVA_OPTS \
        ...
        "$@" "&"
    DEMO_PID=$!
    echo "JBoss Modules Demo PID: $DEMO_PID"
fi
